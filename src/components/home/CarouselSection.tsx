...
export default function CarouselSection() {
  ...
  // Resetear índices cuando cambia la propiedad
  useEffect(() => {
  ...
  const goToPrevImage = () => {
    if (currentProperty?.images && currentProperty.images.length > 1) {
      setActiveImageIndex(activeImageIndex === 0 ? currentProperty.images.length - 1 : activeImageIndex - 1);
    }
  };

  const goToNextImage = () => {
    if (currentProperty?.images && currentProperty.images.length > 1) {
      setActiveImageIndex(activeImageIndex === currentProperty.images.length - 1 ? 0 : activeImageIndex + 1);
    }
  };
  ...
  const currentImage = currentProperty.images?.[activeImageIndex];
    ...
    <section className="relative w-full bg-gradient-to-br from-zinc-50 via-white to-red-50/20 dark:from-zinc-900 dark:via-black dark:to-red-900/10 py-16 lg:py-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="relative aspect-[16/10] lg:aspect-[21/9] rounded-2xl overflow-hidden shadow-2xl group">
            <AnimatePresence mode="wait">
              <motion.div
                key={`${currentProperty.id}-${activeImageIndex}`}
              ...
              >
                <PropertyImage
                  src={currentImage}
                  alt={currentProperty?.title || 'Propiedad'}
                  fill
            ...
            {hasMultipleImages && (
              <>
                ...
                <div className="absolute top-4 right-4 bg-black/70 text-white px-3 py-1 rounded-full text-sm font-medium backdrop-blur">
                  {activeImageIndex + 1} / {currentProperty.images.length}
                </div>
              </>
            )}
